...
namespace Inscription
{
    ...
    template<class Key, class Hash, class Predicate, class Allocator>
    void Scribe<std::unordered_multiset<Key, Hash, Predicate, Allocator>>::Scriven(ObjectT& object, Format::Binary& format)
    {
        if (format.IsOutput())
        {
        ...
        else
        {
            ...
            while (size-- > 0)
            {
                ...
                if (object.count(*constructor.Get()) == 1)
                    format.types.AttemptReplaceTrackedObject(*constructor.Get(), RemoveConst(*emplaced));
            }
        }
    }

    template<class Key, class Hash, class Predicate, class Allocator>
    void Scribe<std::unordered_multiset<Key, Hash, Predicate, Allocator>>::Scriven(const std::string& name, ObjectT& object, Format::Json& format)
    {
        if (format.IsOutput())
        {
    ...
    struct ScribeTraits<std::unordered_multiset<Key, Hash, Predicate, Allocator>, Format>
    {
